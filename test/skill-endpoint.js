...
describe("skill Endpoints", function () {
   ...
   describe("POST /api/skills/add/:user_id TEST", () => {
      context("Given there are users in the database", () => {
         ...
         it("POST /api/skills/add/:id responds with 200 and profiles", () => {
            const testSkill = {
               user_id: 1,
               skill_name: "Postgres",
               skill_level: "mid",
   ...
   describe("ADD /api/skills/edit/:user_id TEST", () => {
      context("Given there are users in the database", () => {
         const testUsers = makeUsersArray();
         const testProfile = makeProfilesArray();
         const testSkills = makeSkillsArray();
         ...
         it("ADD /api/batch/:id adds skills in a batch", () => {
            const testSkill = [
               {
                  user_id: 1,
                  skill_name: "Postgres",
            ...
            return supertest(app)
               .post(`/api/skills/batch/${testSkill.user_id}`)
               .send(testSkill)
               .set("Authorization", authHelper.makeAuthHeader(testUsers[0]))
               .expect(204);
         });
